...
class Editor extends Accessor {
  ...
  get graphics() {
    return this.layers.toArray().flatMap((layer) => layer.graphics.toArray());
  }
          ...
          handles.push(
            view.on("click", async (event) => {
              ...
              const waterResult = graphicResults.find((result) =>
                (result.graphic.symbol as Symbol3D).symbolLayers?.some(
                  (symbolLayer) => symbolLayer instanceof WaterSymbol3DLayer
                )
              );

              const result =
                waterResult ??
                graphicResults.find((result) =>
                  this.graphics.includes(result.graphic)
                );
              ...
              if (result) {
                const operation = new Operation((complete) => {
                  ...
                  const index = (
                    graphic.layer as GraphicsLayer
                  ).graphics.indexOf(graphic);
...
class Operation {
  ...
  constructor(
    operate: (
      complete: (result: Awaited<Operation["promise"]>) => void,
      cancel: () => void
    ) => SketchViewModel
  ) {
    const { promise, resolve, reject } =
      Promise.withResolvers<Awaited<Operation["promise"]>>();
...
export function EditorProvider(props: PropsWithChildren) {
  ...
  function splitAndOrderGraphic(graphic: Graphic, providedIndex?: number) {
    ...
    const index =
      providedIndex ??
      (graphic.layer as GraphicsLayer).graphics.indexOf(graphic);
    if (intersection != null) {
      ...
      layer.graphics.addMany(
        graphicsAfter.map((g) => Graphic.fromJSON(g.toJSON()))
      );
    } else {
      layer.remove(graphic);
    }
  ...
  function splitAndOrderGraphics(result: Awaited<Operation["promise"]>) {
    ...
    const splitAndOrderedGraphics = Object.entries(graphics).flatMap(
      ([i, graphic]) => {
